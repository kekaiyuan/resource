...
共有四个隔离级别：
- 读未提交
	- 即 **脏读**，可以读取其他事务中尚未提交的数据。
- 读已提交
	- 只能读取已经提交的数据。
	- 存在 **不可重复读**，即 事务A 读取了某数据后，其他事务提交修改了该数据，事务A 再次读取该数据发现两次数据不匹配。<br>
		针对 **update** 和 **update**
- 可重复读
	- 保证事务处理的过程中，多次读取同一个数据时，其值都和事务开始时刻是一致的。
	- 存在 **幻读**，事务在开始时读取到 **10** 条数据，在期间其他事务插入了**相同**的 10 条数据，之后事务就会读取到 **20** 条数据。<br>
		针对 **insert** 
		- 原因？<br>
...
常见的是日志系统，事务中的每一个操作都会即时写入日志中，当提交后，系统会把日志里的操作再同步到数据库中。
