...
method stop..
```
仔细阅读源码会发现，明明没有调用 `TimeProxy `类，但是 `TimeProxy `类的 `invoke `方法却被调用了
这就是动态代理。
通过正确地使用Java自带的Proxy类，JVM会自动地帮我们把被代理类和代理类关联起来。
...
> h - 调度方法调用的调用处理函数
	该方法有三个参数
	1. ClassLoader
		   生成的动态代理应该用哪个 ClassLoader 装载，一般设置为被代理类的 ClassLoader
	2. Class数组
		   代理类实现的接口列表，这就要求被代理类必须有一个接口
	3. 
