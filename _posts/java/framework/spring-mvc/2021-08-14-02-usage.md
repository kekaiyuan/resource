...
结果
```java
User{id=1, name='kky', age=24, gender='男', address=Address{province='云南', city='大理', town='下关'}}
```
...
### 注意
如果项目中配置了多个过滤器，那么字符编码过滤器必须在最前面（谁在前面先执行谁），否则失效。
...
public class Person {
    ...
    //...省略 getter/setter 方法和 toString 方法
}
```
...
```
该 Controller 会将所有的错误信息存入一个 Map 中，并把这个 Map 放入 Model。<br>
根据是否有错误信息，返回的页面不一样。
- 有错误，返回原页面，让用户重新输入。
- 无错误，跳转到成功页面。
