...
![](\images\posts\java\framework\spring-mvc\01-introduce\project.jpg)<br>
...
1. 创建一个 `org.springframework.web.servlet.DispatcherServlet` 的对象 `springmvc`，即前端控制器。<br>
	当用户发送请求时，会先送到这里。
2. 设置 `springmvc` 的初始化参数，通过读取 springmvc.xml 中的配置进行初始化。
