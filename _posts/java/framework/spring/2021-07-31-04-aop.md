...
```java
package com.mashibing.services;
...
而如果是动态的，那么程序将在执行过程中，将 LogUtil 类的方法切入到某个类的方法中（程序运行前并未写入该类）。
...
`@Before` `@After` `@AfterThrowing` `@AfterReturning`<br>
根据字面意思理解即可：方法执行之前、方法执行之后、方法抛出异常之后、方法返回值之后<br>
表明了该方法会切入到什么**位置**。
...
- 设置切面类中的方法是什么时候在哪里执行
  ...
  public class LogUtil {
       ...
       */
      @Before("execution( public int com.kky.services.MyCalculator.*(int,int))")
      public static void start(){
  ...
  //        System.out.println(method.getName()+"方法开始执行，参数是："+ Arrays.asList(objects));
      ...
      @AfterReturning("execution( public int com.kky.services.MyCalculator.*(int,int))")
      public static void stop(){
  ...
  //        System.out.println(method.getName()+"方法执行结束，结果是："+ Arrays.asList(objects));
      ...
      @AfterThrowing("execution( public int com.kky.services.MyCalculator.*(int,int))")
      public static void logException(){
  //        System.out.println(method.getName()+"方法出现异常："+ e.getMessage());
          System.out.println("方法出现异常：");
      }
  
      @After("execution( public int com.kky.services.MyCalculator.*(int,int))")
      public static void end(){
  //        System.out.println(method.getName()+"方法结束");
...
```java
  import com.kky.services.Calculator;
...
class com.kky.services.MyCalculator$$EnhancerBySpringCGLIB$$1f93b605类型
...
```java
public Integer com.kky.service.MyCalculator.add(Integer,Integer)
```
...
```java
    @AfterReturning(value = "execution( public int com.kky.services.MyCalculator.*(int,int))",returning = "result")
...
```java
    @AfterThrowing(value = "execution( public int com.kky.services.MyCalculator.*(int,int))",throwing = "exception")
...
```java
    @After("execution( public int com.kky.services.MyCalculator.*(int,int))")    private int end(JoinPoint joinPoint,String aa){//        System.out.println(method.getName()+"方法结束");        String name = joinPoint.getSignature().getName();        System.out.println(name+"方法结束");        return 0;    }
```
