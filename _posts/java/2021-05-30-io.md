...
# Java.Io中的常用类<sup id="a1">[[1]](#f1)</sup>
...
# 注意事项
- 每个Io流在使用后都必须调用close()方法关闭Io流，否则会造成资源泄露
	- 正常的写法
	- ```java
	...
	- try-with-resources
		- ```java
		  ...
		  ```
		- 编译器会自动补全close()，反编译.class文件可以印证这一点。
- 如果是带缓冲区的OutputStream或Writer，当缓冲区未满时，使用flush()刷新输出流并强制缓冲区的输出字节被写出。close()方法也首先调用了flush()来保证所有输出都被写入。
