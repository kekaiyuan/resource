...
然后考虑以下这个非常非常意外的巧合：
1. 线程 1 访问该单例，发现没有创建单例，于是在该线程中，执行创建单例的操作。
2. 系统将 `INSTANCE = new Mgr06();` 的指令重排了。<br>
	于是线程 1 先申请了内存，将 INSTANCE 指向了该内存。
3. 在线程 1 初始化成员变量前，线程切换了，线程 2 也要使用该单例模式。
4. 线程 2 发现 `INSTANCE` 不是 `null` ，它已经指向了某块内存。<br>
		于是线程 2 开始直接使用 `INSTANCE` 。
5. 但是 `INSTANCE` 的成员变量并没有初始化，此时它们都是默认值（例如 int 类型的变量默认值是 0 ）。<br>
			所以线程 2 使用 `INSTANCE` 执行了一系列奇奇怪怪的操作，程序出错。
			
这是一个极难复现的 BUG，要在百万级别的并发下才会出现。<br>
...
m start
```
线程死循环，running[0] 并不是线程间可见的

数组如此，类对象也如此。<br>
只要是引用类型，volatile 只能保证当引用类型本身的值是线程间可见的。<br>
但是引用类型的**内部字段**的值是**线程不可见**的。
...
```
该程序的目的是创建 10 个线程，每个线程在 count 上执行 10000 次 ++ 操作
预期输出是 count = 100000
在没有锁的情况下，实际输出是多少？
