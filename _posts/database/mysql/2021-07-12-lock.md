...
| :-- | :---------------------------------------------------------- | :-- | :---------------------------------------------------------- | :-- |
| T1 |       ock table mylock read;  | 获得表的read锁定 |         |                                                              | |
| T2 |   select * from mylock;   | 执行成功<br>当前session可以查询该表记录 | select * from mylock;   | 执行成功<br>当前session可以查询该表记录 |
|      | select * from person | 当前session不能查询没有锁定的表 | 当前session可以查询或者更新未锁定的表<br />select * from mylock<br />insert into person values(1,'zhangsan'); |
...
![表格](/images/attachments/1626080537516.table.html)
